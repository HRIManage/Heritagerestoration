...
const BLOG_ARTICLES = [
  {
    ...
    excerpt: "Learn how to identify water damage early, prevent mold growth, and understand the restoration process."
  },
  {
    title: "Fire Damage Restoration: What to Expect",
    excerpt: "A comprehensive guide to the fire restoration process, from emergency response to final reconstruction."
  },
  {
    title: "Storm Damage: Preparation and Recovery",
    excerpt: "Prepare your home for severe weather and understand the steps for quick recovery after storm damage."
  },
  {
    title: "Insurance Claims: How to Get Full Coverage",
    excerpt: "Tips for working with your insurance adjuster and ensuring your claim is approved for full restoration."
  },
];
    ...
    <div className="min-h-screen bg-white text-[#3F4143] flex flex-col" style={{ fontFamily: "'Lato', sans-serif" }}>
      ...
      <Dialog open={showFaqModal} onOpenChange={setShowFaqModal}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-[#3F4143]">Frequently Asked Questions</DialogTitle>
            <DialogDescription>Common questions about our restoration services</DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              {FAQ_ITEMS.map((item, idx) => (
                <div key={idx} className="border-b border-gray-200 pb-4 last:border-0">
                  ...
                  <p className="text-[#3F4143]/80 text-sm leading-relaxed">{item.a}</p>
                </div>
              ))}
            </div>
          </ScrollArea>
      ...
      <Dialog open={showBillOfRightsModal} onOpenChange={setShowBillOfRightsModal}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-[#3F4143]">Homeowner Bill of Rights</DialogTitle>
            <DialogDescription>Know your rights after a property loss</DialogDescription>
          </DialogHeader>
          <ScrollArea className="h-[60vh] pr-4">
            <div className="space-y-3">
              {BILL_OF_RIGHTS.map((right, idx) => (
                <div key={idx} className="flex gap-3">
                  ...
                  <p className="text-[#3F4143]/80 text-sm leading-relaxed pt-0.5">{right}</p>
                </div>
              ))}
            </div>
          </ScrollArea>
      ...
      <Dialog open={showBlogsModal} onOpenChange={setShowBlogsModal}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-[#3F4143]">Restoration Guides & Articles</DialogTitle>
            <DialogDescription>Learn more about restoration and recovery</DialogDescription>
          </DialogHeader>
          <ScrollArea className="h-[60vh] pr-4">
            <div className="space-y-4">
              {BLOG_ARTICLES.map((article, idx) => (
                <div key={idx} className="border border-gray-200 rounded-lg p-4 hover:border-[#7BB843] hover:shadow-md transition-all duration-300 cursor-pointer">
                  <h3 className="font-bold text-[#3F4143] mb-2">{article.title}</h3>
                  <p className="text-[#3F4143]/70 text-sm leading-relaxed">{article.excerpt}</p>
                  <button className="text-[#7BB843] font-semibold text-sm mt-3 hover:text-[#3F4143] transition-colors">Read More →</button>
                </div>
              ))}
            </div>
          </ScrollArea>
