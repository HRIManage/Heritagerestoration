...
export default function Header({
  ...
  const serviceItems = [
    {
      ...
      sub: "Soot, shoring & full rebuild",
      href: "/services/fire-restoration",
    },
    {
      label: "Water Restoration",
      sub: "Extraction, dryout & drywall",
      href: "/services/water-restoration",
    },
    {
      label: "Storm Restoration",
      sub: "Board-up, tarping & framing",
      href: "/services/storm-recovery",
    },
    {
      label: "Contents Services",
      sub: "Pack-out, clean & storage",
  ...
  const resourceItems = [
    {
      label: "FAQ",
      sub: "Common insurance & process questions",
      href: "/resources/faq",
      handler: onFaqClick,
    },
    {
      label: "Blogs",
      sub: "Guides & restoration articles",
      href: "/resources/blog",
      handler: onBlogsClick,
    },
    {
      label: "Homeowner Bill of Rights",
      sub: "Know your rights after a loss",
  ...
  const navLinkCls =
    "px-4 py-2 text-[18px] font-bold text-white/90 hover:text-[#8DBD42] transition-colors relative group whitespace-nowrap tracking-wide flex items-center gap-1.5";
  ...
  return (
    <>
        ...
        <div className="max-w-[1280px] mx-auto px-6 flex flex-wrap items-center justify-between h-11">
          <div className="flex items-center gap-5">
            <a
              ...
              className="flex items-center gap-1.5 text-[15.5px] font-bold hover:text-[#3F4143] transition-colors"
            >
              <Phone size={15} />
            ...
            <span className="hidden lg:block text-[15px] font-semibold tracking-wide text-white/90">
      ...
      {/* ── MAIN NAV ── */}
      <nav
        className={`fixed left-0 right-0 z-40 transition-all duration-300 ${scrolled ? "top-0 shadow-lg" : "top-[44px]"}`}
      >
        <div className="bg-[#3F4143] border-b border-white/10 relative h-[98px]">
          <div
            className="absolute left-0 top-0 bottom-0 bg-[#8DBD42] z-10"
            style={greenAccentStyle}
          />
          <div
            className="absolute left-0 top-0 bottom-0 bg-white z-20 flex items-center"
            style={logoContainerStyle}
          >
            <Link
              href="/"
              className="absolute h-[84px] w-auto flex items-center"
              style={logoStyle}
            >
              <img
                src={LOGO}
          ...
          <div className="max-w-[1280px] mx-auto px-6 flex items-center justify-between h-full relative z-30">
                  ...
                  {desktopServicesOpen && (
                    ...
                    >
                      {serviceItems.map(item => (
                        ...
                        >
                          <span className="text-white font-bold text-sm group-hover:text-[#8DBD42] transition-colors">
                            {item.label}
                          </span>
            ...
            <div className="hidden lg:flex items-center ml-auto">
              <a
                href="tel:+13608511407"
                className="flex items-center gap-2 bg-[#8DBD42] hover:bg-[#72a232] text-white px-5 py-2.5 rounded font-bold text-base transition-all duration-200 shadow-sm hover:shadow-md"
              >
            ...
            <button
              ...
              className="lg:hidden p-2 text-white hover:text-[#8DBD42]"
            >
          ...
          <AnimatePresence>
            {mobileOpen && (
              <motion.div
                ...
                className="lg:hidden bg-[#3F4143] border-t border-white/10 overflow-hidden shadow-lg"
              >
                <div className="flex flex-col divide-y divide-white/10 max-h-[70vh] overflow-y-auto">
                  <Link
                    href="/"
                    className="px-6 py-4 text-white font-bold text-base hover:text-[#8DBD42]"
                  ...
                  <div>
                    <button
                      ...
                      className="w-full text-left px-6 py-4 text-white font-bold text-base flex justify-between items-center"
                    >
                      <span>Services</span>
                      <ChevronDown
                        size={16}
                        className={`text-[#8DBD42] transition-transform duration-200 ${mobileServicesOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    <AnimatePresence>
                      {mobileServicesOpen && (
                        ...
                        >
                          {serviceItems.map(item => (
                            <Link
                              key={item.label}
                              href={item.href}
                              className="block px-10 py-3 text-sm text-white/90 font-semibold hover:text-[#8DBD42]"
                              onClick={() => setMobileOpen(false)}
                            >
                              {item.label}
                            </Link>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <ChevronDown
                        size={16}
                        className={`text-[#8DBD42] transition-transform duration-200 ${mobileResourcesOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    <AnimatePresence>
                      {mobileResourcesOpen && (
                        ...
                        >
                          {resourceItems.map(item => (
                            <Link
                              key={item.label}
                              href={item.href}
                              className="w-full text-left block px-10 py-3 text-sm text-white/90 font-semibold hover:text-[#8DBD42]"
                              onClick={e => {
                                handleResourceClick(e, item);
                                setMobileOpen(false);
                              }}
                  ...
                  <a
                    href="tel:+13608511407"
                    className="m-4 flex items-center justify-center gap-2 bg-[#8DBD42] text-white py-4 font-bold text-base shadow-sm"
                  >
                    <Phone size={16} /> +1(360)851-1407
                  </a>
                </div>
